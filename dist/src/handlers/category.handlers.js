 ...
 */
async function smartCategorySearch(query, maxResults = 10) {
    ...
    for (const category of categories) {
        ...
        for (const level of levels) {
            ...
            if (level.name === '대분류')
                levelBonus = 50;
            else if (level.name === '중분류')
                levelBonus = 30;
            else if (level.name === '소분류')
                levelBonus = 20;
            else if (level.name === '세분류')
                levelBonus = 10;
 ...
 * 카테고리 검색 핸들러
 */
export const findCategoryHandler = async ({ query, max_results = 10 }) => {
    try {
...
export const categoryToolHandlers = {
