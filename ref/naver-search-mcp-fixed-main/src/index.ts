...
// 프롬프트 목록을 반환하는 핸들러 등록 (빈 배열 반환)
server.setRequestHandler(ListPromptsRequestSchema, async () => {
  console.error("[MCP] ListPromptsRequestSchema 핸들러 호출됨");
  return {
    prompts: [],
  };
});

// 리소스 목록을 반환하는 핸들러 등록 (빈 배열 반환)
server.setRequestHandler(ListResourcesRequestSchema, async () => {
  console.error("[MCP] ListResourcesRequestSchema 핸들러 호출됨");
  return {
    resources: [],
  };
});
